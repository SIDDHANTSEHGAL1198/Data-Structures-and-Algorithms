...
node *bst_search(node *root,int ele)
{
    if(root==NULL)
    {
        return NULL;
    ...
    if(root->data>ele)
    {
        bst_search(root->left,ele);
    }
    if(root->data<ele)
    {
        bst_search(root->right,ele);
    }
...
int main()
{
    ...
    cout<<"Enter 5 for Searching an element"<<endl;
